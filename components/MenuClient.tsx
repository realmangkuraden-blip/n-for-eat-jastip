...
export default function MenuClient(){
 ...
 return <div className="safe-bottom container py-8 md:py-10">
  <div className="mb-6"><div className="text-sm font-black text-[#ff6b45]">GO JASTIP 4N</div><h1 className="mt-1 text-4xl font-black">Pilih Menu 🍴</h1><p className="mt-2 text-[#6d625b]">Pilih makanan, atur jumlah, lalu lihat keranjang di bawah.</p></div>
  <div className="sticky top-[64px] z-10 -mx-1 mb-7 rounded-3xl border bg-[#fffaf1]/95 p-3 backdrop-blur">
   ...
   <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{cats.map(c=><button key={c} onClick={()=>setCat(c)} className={`shrink-0 rounded-full px-4 py-2 text-sm font-black transition ${cat===c?'bg-[#ff6b45] text-white shadow-sm':'border bg-white'}`}>{c}</button>)}</div>
  </div>
  ...
  <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{filtered.map(p=>{const qty=cart[p.id]||0;return <article className="product-card card overflow-hidden" key={p.id}>
   <Link href={"/menu/"+p.slug} className="block"><div className="flex h-48 items-center justify-center bg-[#fff0a8] text-7xl">{p.image_url?<img src={p.image_url} alt={p.name+' - GO JASTIP 4N'} className="h-full w-full object-cover" loading="lazy"/>:emojiFor(p.categories[0]?.name)}</div></Link>
   <div className="p-5"><div className="mb-1 text-xs font-black uppercase tracking-wide text-[#7d7067]">{p.categories.length?p.categories.map(c=>c.name).join(' · '):'Menu'}</div><Link href={"/menu/"+p.slug} className="text-xl font-black hover:text-[#ff6b45]">{p.name}</Link>{p.description&&<p className="my-2 line-clamp-2 text-sm text-[#6d625b]">{p.description}</p>}<div className="text-xs text-[#7d7067]">Harga {rupiah(Number(p.selling_price))} + jastip {rupiah(Number(p.jastip_fee))}</div>
   <div className="mt-4 flex items-center justify-between gap-3"><div><b className="text-lg text-[#ff6b45]">{rupiah(Number(p.selling_price)+Number(p.jastip_fee))}</b><div className="text-[11px] text-[#7d7067]">per porsi/item</div></div>
   {qty===0?<button onClick={()=>add(p.id)} disabled={!p.is_available} className="btn btn-primary gap-1 px-5"><Plus size={18}/> Tambah</button>:<div className="flex items-center gap-2 rounded-full border bg-[#e8f8f5] p-1"><button aria-label={`Kurangi ${p.name}`} onClick={()=>dec(p.id)} className="rounded-full bg-white p-2 shadow-sm"><Minus size={15}/></button><b className="min-w-7 text-center">{qty}</b><button aria-label={`Tambah ${p.name}`} onClick={()=>add(p.id)} className="rounded-full bg-[#ff6b45] p-2 text-white shadow-sm"><Plus size={15}/></button></div>}</div></div>
  </article>})}</div>
  {items.length>0&&<aside className="fixed bottom-4 left-1/2 z-30 w-[calc(100%-20px)] max-w-xl -translate-x-1/2 rounded-3xl border bg-white/95 p-4 shadow-2xl backdrop-blur"><div className="flex items-center justify-between"><div className="flex items-center gap-3"><div className="rounded-2xl bg-[#fff0a8] p-3"><ShoppingBag size={20}/></div><div><b className="block">{totalQty} item di keranjang</b><span className="text-sm text-[#6d625b]">{rupiah(total)}</span></div></div><Link href="/checkout" className="btn btn-primary gap-1 px-5">Checkout <ChevronRight size={17}/></Link></div><div className="mt-3 max-h-24 space-y-1 overflow-auto border-t pt-2">{items.map(x=><div key={x.p.id} className="flex items-center justify-between text-sm"><span className="truncate pr-3">{x.p.name} × {x.qty}</span><button aria-label={`Hapus ${x.p.name}`} onClick={()=>setCart(c=>{const y={...c};delete y[x.p.id];return y})}><Trash2 size={15} className="opacity-50"/></button></div>)}</div></aside>}
 </div>;
}
