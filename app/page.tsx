...
export default async function Home() {
  ...
  return <main>
    ...
    <section className="container py-10"><h2 className="text-3xl font-black">Menu Favorit</h2><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{products?.map(p=><div className="card p-5" key={p.id}><div className="text-5xl">🍴</div><h3 className="mt-3 font-black">{p.name}</h3><p className="text-sm opacity-60">{p.category?.name || 'N for Eat'}</p><b className="text-[#e97827]">{rupiah(Number(p.selling_price) + Number(p.jastip_fee))}</b></div>)}</div></section>
